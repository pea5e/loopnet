# from impulse import Solver as captcha
# input()
import sys
import subprocess
try:
...
class Loopnet:
    ...
    def contact(self,message):
        ...
        try:
            ...
            sleep(3)
            # undetect = captcha(self.browser)
            # undetect.run()
            # self.browser.get_screenshot_as_file("screenshot.png")
            captcha = self.browser.find_element(By.ID,"recaptchaContainer_SideBrokerContact").find_element(By.TAG_NAME,"div")
            # print(captcha.rect,captcha.location)
            curs = ActionChains(self.browser)
            curs.move_to_element(captcha).move_by_offset(12+7, 23+7).click().perform()
                    # 52x74
                    # 28x28
                    # 12-23
            # captchabutton = self.browser.find_element(By.CLASS_NAME,"recaptcha-checkbox-border")
            # print(captchabutton.location)
            # print(captchabutton.accessible_name)
            # print(captchabutton.rect)
            # print(captchabutton.is_displayed())
            # captchabutton.click()
            input()
    ...
    def is_article_available(self):
        try:
            if "This listing is not currently being advertised on LoopNet.com" in self.browser.find_element(By.CLASS_NAME,"listing-not-visible-header").text :
                return False
        except:
            pass
        # finally:
    ...
    def getarticleinfos(self,url):
        ...
        return info
    @classmethod
    def sent_article(meth,url,index):
        # print("https://loopnet.salimalaoui.repl.co/?sent="+re.findall("/(\d+)/$",url)[0]+"&ind="+str(index))
        get("https://loopnet.salimalaoui.repl.co/?sent="+re.findall("/(\d+)/$",url)[0]+"&ind="+str(index))
...
def cont1(chrome,chromeuser):
    ...
    with open("data.json",'w') as f:
        f.write(str(jss).replace("{'",'{"').replace(": '",': "').replace("',",'",').replace("':",'":').replace(", '",', "').replace("'}",'"}'))
        # f.write("{")
        # jsson = list()
        # for key in jss:
        #     jsson.append('"'+key+'":"'+jss[key]+'"')
...
def send(message):
    ...
    mybot.login()
    global lmessage
    lmessage = message.strip()
    # print(message)
    jss["lastmessage"] = message.strip()
    with open("data.json",'w') as f:
        f.write(str(jss).replace("{'",'{"').replace(": '",': "').replace("',",'",').replace("':",'":').replace(", '",', "').replace("'}",'"}'))
        # f.write("{")
        # jsson = list()
    ...
    for i in req:
        for ids in js:
            if ids["id"]==i:
                article = ids
                break
        Broker = article["owner"].split(' ')[0]
        Brokerage = article["groupname"]
        Name = article["name"]
        City = article["city"]
        print(article["url"])
        ...
        # input()
        # Loopnet.sent_article(article["url"],mybot.last_failed+1)
        # if mybot.last_failed:
        #     req.remove(i)
    with open("Requests sent count.txt",'a') as f:
        f.write("\n"+str(date).split('.')[0]+'-\t'+str(len(req)))
...
while True:
    ...
    while run:
        ...
        num = Tk.StringVar()
        number = ttk.Frame(screen)
        ttk.Entry(number,textvariable=num).grid(row=0,column=1)
        ttk.Button(number,text="Send Number Of Requests",command=lambda:sendnum(num.get())).grid(row=0,column=2)
        number.pack()
    ...
    ttk.Label(option,text="{Broker} for proprety owner",foreground="blue").grid(row=0,column=0,padx=(10,10))
    ttk.Label(option,text="{Brokerage} for group name",foreground="blue").grid(row=0,column=1,padx=(10,10))

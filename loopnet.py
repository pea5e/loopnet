...
class Loopnet:
    ...
    @classmethod
    def openbrowserasuser(mth,userdata):
        return r'"C:\Program Files\Google\Chrome\Application\chrome.exe"  --user-data-dir="{0}" "https://www.loopnet.com/"'.format(userdata)
...
def cont():
    ...
    clear()
    sync = Loopnet("chromedriver.exe",r"C:\Users\{0}\AppData\Local\Google\Chrome\User Data\Default".format(os.getlogin()))#,False)
    sync.login()
    sync.advancedsearch()
    sync.forsale()
    sync.chooseType()
...
while jss["state"]=="logged-out":
    ...
    log = Loopnet.openbrowserasuser(r"C:\Users\{0}\AppData\Local\Google\Chrome\User Data\Default".format(os.getlogin()))
    ...
    contin = ttk.Button(screen,text="Continue",style="CON.TButton",command=cont)
    comm.pack()
    contin.pack()
    screen.mainloop()
...
def send(message):
    ...
    mybot = Loopnet("chromedriver.exe",r"C:\Users\{0}\AppData\Local\Google\Chrome\User Data\Default".format(os.getlogin()),headless)
    mybot.login()
    global lmessage
    lmessage = message
    ...
    for i in req:
        ...
        # if article["price"] in [None,'']:
        #     message += jss["priceless"]
...
#loopnet.com/header/id
#{"SubjectId":"7b98a44c-cb5f-47a1-71d8-00000185889a"}